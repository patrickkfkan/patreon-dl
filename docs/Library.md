...
| `stopOn`      | Sets the condition to stop the downloader. Values can be: <ul><li>`never`: do not stop; run till the end.</li><li>`previouslyDownloaded`: stop on encountering a post or product that was previously downloaded. Requires enabling `useStatusCache`.</li><li>`publishDateOutOfRange`: stop on encountering a post or product published outside the date range set by `include.postsPublished` or `include.productsPublished` (as the case may be).</li></ul> Deprecated values:<ul><li>`postPreviouslyDownloaded`: replaced by `previouslyDownloaded`.</li><li>`postPublishDateOutOfRange`: replaced by `publishDateOutOfRange`.</li></ul>Default: `never`|
